...
#In order to be a univaersal approximator, the network must have non-linearities: relu
#I am training the network to approximate sin x  on [-2*pi, 2*pi]
#Results: very close approximation to six(x), the 'curve' is not very smooth
import os
import math
import time
import random
...
save_path = path + "\\univ_relu\\classifier.ckpt"
...
h_actv_2 = tf.nn.relu(h_2, name='h_actv_2')
...
h_actv_3 = tf.nn.relu(h_3, name='h_actv_3')
...
if os.path.isfile(path + "\\univ_relu\\checkpoint"):
...
save_path = saver.save(sess, path + "\\univ_relu\\classifier.ckpt")
